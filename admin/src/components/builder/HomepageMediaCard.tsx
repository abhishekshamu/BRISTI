...
interface HomepageMediaCardProps {
  value?: string;
  onChange: (url: string, meta?: HomepageMediaMeta) => void;
...
/**
 * Large-preview media card used ONLY by the Homepage Builder properties panel.
 * Reuses the exact same upload / replace / verify / crop / library services and
 * dialogs as the rest of the admin, so behaviour is identical.
 */
export default function HomepageMediaCard({
  value = '',
  onChange,
  label,
  ratio,
  ...
  const validExtRe = useMemo(
    () => (allowsVideo ? /^(jpg|jpeg|png|webp|svg|gif|avif|mp4|webm)$/i : /^(jpg|jpeg|png|webp|svg|gif|avif)$/i),
    [allowsVideo]
  );
  ...
  const handleFiles = useCallback(
    ...
    [folder, handleUploaded, allowsVideo, validExtRe]
  );
  ...
  const isVideo = allowsVideo || isVideoValue(value);
        ...
        <div className="mb-3 flex items-center justify-between gap-3">
          <label className="admin-label">
            ...
            {ratioInfo && ratioInfo.w > 0 && (
              <span className="ml-2 text-[11px] font-normal text-slate-400">— {ratioInfo.w}:{ratioInfo.h}</span>
            )}
          </label>
          {media && value && (
        ...
        {imgState === 'empty' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-slate-400 p-6 text-center">
            <div className="w-14 h-14 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-center">
              {isVideo ? <Video className="w-6 h-6" /> : <Upload className="w-6 h-6" />}
            </div>
            <div>
              <p className="text-[13px] font-medium text-slate-500 dark:text-slate-400">No {isVideo ? 'video' : 'image'} yet</p>
              <p className="text-[11.5px] mt-1">Drag &amp; drop, upload, paste a URL, or pick from the library</p>
            </div>
          </div>
        )}
        {imgState === 'loading' && (
...
<img src={resolveMediaUrl(value) ?? ''} alt="Preview" className="w-full h-full object-cover" draggable={false} />
      ...
      {/* Actions — equal-size buttons, no overlap */}
      <div className="mt-4 space-y-2.5">
        <div className="grid grid-cols-2 gap-2.5">
          <button
            ...
            disabled={busy}
            className={`admin-btn-primary !h-11 !rounded-xl px-4 text-[13px] flex items-center justify-center gap-2 disabled:opacity-60 ${value ? '' : 'col-span-2'}`}
          >
          ...
          {value && (
            <button
              ...
              onClick={openFilePicker}
              disabled={busy}
              className="admin-btn-primary !h-11 !rounded-xl px-4 text-[13px] flex items-center justify-center gap-2 disabled:opacity-60"
            >
              <Upload className="w-4 h-4" />
              Replace
            </button>
          ...
          <button
            ...
            disabled={busy}
            className="admin-btn-secondary !h-11 !rounded-xl px-4 text-[13px] flex items-center justify-center gap-2 disabled:opacity-60"
          >
          ...
          {value && (
            <button
              type="button"
              onClick={copyUrl}
              className="admin-btn-secondary !h-11 !rounded-xl px-4 text-[13px] flex items-center justify-center gap-2"
            >
        ...
        {value && (
          <button
            type="button"
            onClick={removeImage}
            className="admin-btn-ghost !h-11 !rounded-xl px-4 text-[13px] flex items-center justify-center gap-2 w-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-700 dark:hover:text-red-300"
          >
      ...
      {urlOpen && (
        <div className="mt-3 flex items-center gap-2">
          <input
            ...
            className="admin-input !h-11 !rounded-xl !bg-[#FAFAFA] dark:!bg-slate-900/60 px-4 text-sm flex-1 min-w-0"
            aria-label="Media URL"
            autoFocus
          />
          <button
            type="button"
            onClick={() => void applyUrl(urlInput)}
            disabled={busy || !urlInput.trim()}
            className="admin-btn-primary !h-11 !rounded-xl px-5 text-[13px] shrink-0 disabled:opacity-60"
          >
            Apply
          </button>
          <button
            ...
            className="w-11 h-11 shrink-0 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 flex items-center justify-center transition-colors"
      ...
      {media && value && (
        <p className="mt-3 text-[11.5px] text-slate-400 dark:text-slate-500 flex items-center gap-1.5">
